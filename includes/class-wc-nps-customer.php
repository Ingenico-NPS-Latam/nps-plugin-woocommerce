  ...
  protected static function get_nps_logging() {
    ...
    // $this->log("ENTRY get_customer_data ====> this->customer_data[" . var_export(@$this->customer_data,true) . "] this->get_id[" . var_export($this->get_id(),true) . "] get_transient[" . var_export(get_transient( 'nps_customer_' . $this->get_id()), true)  . "]");
      ...
      $this->set_customer_data( $response_retrieveCustomer );
		}else if($this->customer_data && false === ($cards = get_transient( 'nps_cards_' . $this->get_id() ))) {
    ...
    $requestCreateCustomerParameters = array(
      ...
      'psp_Address' => array(
          ...
          'HouseNumber'=>$billing_address_1,
          'AdditionalInfo'=>$billing_address_2,
          'City'=>$billing_city,
    ...
    $this->log("Success: Customer created - Customer ID: {$response_createCustomer->psp_CustomerId} - Reason: {$response_createCustomer->psp_ResponseMsg}", true); 
		...
		return $response->psp_CustomerId;
	}
    ...
    $this->log( 'Processing createPaymentMethod response: ' . print_r( $response_createPaymentMethod, true ), WC_Nps_Customer::get_nps_logging() );
      ...
      $this->log("Error: On createPaymentMethod for merchant {$this->merchant_id} - Reason: " . @$response_createPaymentMethod->psp_ResponseExtended, true);           
